...
with which you can compile documents that blend text and code into your desired
format like so:
...
compiled in LaTeX, and often using our [very own styles](https://github.com/datamade/datamade-latex)
(private link). `pweave` weaves `ptexw` files - LaTeX markup with noweb code
chunks – to `tex` files by default.
...
If you need to override defaults, [you can](http://mpastell.com/pweave/script.html).
...
Need plaintext? Start with an `mdw` file. You can input plain old text,
`noweb` codeblocks, and (optionally) HTML markup (use `-f html` to compile
HTML) or markdown (use `-f markdown` to compile markdown, or `-f md2html`
to compile HTML).
...
as plaintext rather than `\verbatim`. To set global code chunk options, `update`
the appropriate options in `pweave.rcParams`, like so:
